...
public class Silver_1927 {
    ...
    public static int delete(){
        ...
        while (index*2 < heap.size()){ // 노드가 루트가 아닐 경우
            ...
            if(changeIndex + 1 < heap.size() && changeValue > heap.get(changeIndex+1)){ // 오른쪽 노드가 존재 하면서, 왼쪽 노드보다 오른쪽 노드가 값이 작을 때
                changeValue = heap.get(changeIndex+1); // 자식 오른쪽 노드 인덱스
                changeIndex += 1; // 오른쪽 노드 값
            }
            if(heap.get(index) < changeValue) break; // 자식 노드보다 부모 노드 값이 더 작으면 break
